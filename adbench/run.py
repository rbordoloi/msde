    ...
    def run(self, dataset=None, clf=None):
        ...
        for i, params in tqdm(enumerate(experiment_params)):
            ...
            if clf is None:
                ...
                # store and save the result (AUC-ROC, AUC-PR and runtime / inference time)
                df_AUCROC.loc[i, self.model_name] = metrics['aucroc']
                df_AUCPR.loc[i, self.model_name] = metrics['aucpr']
                df_time_fit.loc[i, self.model_name] = time_fit
                df_time_inference.loc[i, self.model_name] = time_inference
